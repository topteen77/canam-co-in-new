...
import { Lead, FollowUp, Contact, User } from '../types';

interface FullScreenTrainingProps {
  userRole: string;
  currentUser: string;
  leads: Lead[];
  availableUsers: User[];
...
export const FullScreenTraining: React.FC<FullScreenTrainingProps> = ({
  userRole,
  currentUser,
  leads,
  availableUsers
      ...
      content: (
        <div className="space-y-8">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-8 rounded-xl border border-blue-200">
            <h3 className="text-2xl font-bold text-blue-800 mb-4">Welcome to Canam Marketing CRM</h3>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="bg-white p-8 border rounded-xl shadow-sm">
              <h4 className="text-xl font-semibold text-gray-800 mb-4">🏗️ System Architecture</h4>
            ...
            <div className="bg-white p-8 border rounded-xl shadow-sm">
              <h4 className="text-xl font-semibold text-gray-800 mb-4">🎭 Your Role: {userRole}</h4>
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-8 rounded-xl border border-yellow-200">
            <h4 className="text-xl font-semibold text-yellow-800 mb-6">📊 Current System Statistics</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center p-6 bg-white rounded-lg shadow-sm">
                <div className="text-4xl font-bold text-yellow-700 mb-2">{safeLeads.length}</div>
                <div className="text-sm text-yellow-600 font-medium">Total Leads</div>
                <div className="text-xs text-gray-500 mt-1">Active in system</div>
              </div>
              <div className="text-center p-6 bg-white rounded-lg shadow-sm">
                <div className="text-4xl font-bold text-yellow-700 mb-2">{(Array.isArray(availableUsers) ? availableUsers : []).length}</div>
                <div className="text-sm text-yellow-600 font-medium">Team Members</div>
                <div className="text-xs text-gray-500 mt-1">Registered users</div>
              </div>
              <div className="text-center p-6 bg-white rounded-lg shadow-sm">
                <div className="text-4xl font-bold text-yellow-700 mb-2">{sampleFollowUps.length}</div>
                <div className="text-sm text-yellow-600 font-medium">Active Follow-ups</div>
      ...
      content: (
        <div className="space-y-8">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-8 rounded-xl border border-green-200">
            <h3 className="text-2xl font-bold text-green-800 mb-4">Leads Management Overview</h3>
...
[Image of Lead management process flow]
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div>
              <h4 className="text-xl font-semibold text-gray-800 mb-6">📋 Lead Status Types</h4>
              <div className="space-y-3">
                {[
                  'New', 'In Pipeline', 'ICP Qualified', 'Portal Deactivated', 
                  'Onboarded', 'Lost', 'MOU Signature Pending', 'Agent Portal Created', 'Agent Portal Reactivated'
                ].map(status => (
                  <div key={status} className="flex items-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
            ...
            <div>
              <h4 className="text-xl font-semibold text-gray-800 mb-6">🏆 Agent Categories</h4>
              <div className="space-y-3">
                {[
                  'Platinum', 'Diamond', 'Gold', 'Silver', 'Bronze', 'Beginner'
                ].map(category => (
                  <div key={category} className="flex items-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
          ...
          {sampleLeads.length > 0 && (
            <div className="bg-white border-2 border-gray-200 rounded-xl p-8">
              <h4 className="text-xl font-semibold text-gray-800 mb-6">📊 Live Lead Examples</h4>
              <div className="space-y-6">
                {sampleLeads.map(lead => {
                  const safeContacts = Array.isArray(lead.contacts) ? lead.contacts : [];
                  return (
                    <div key={lead.id} className="border-l-4 border-blue-500 pl-6 py-4 bg-gray-50 rounded-lg">
                      <div className="flex justify-between items-start">
                        <div>
                          <h5 className="text-lg font-semibold text-gray-800 mb-2">{lead.agencyName}</h5>
                          <p className="text-gray-600 mb-2">
                            Status: <span className="font-semibold text-blue-600">{lead.status}</span> | 
                            Category: <span className="font-semibold text-yellow-600">{lead.agentCategory}</span>
                          </p>
                          {lead.accountManager && (
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm text-gray-500">Contacts: {safeContacts.length}</p>
                        </div>
                      </div>
                      {safeContacts.length > 0 && safeContacts[0] && (
                        <div className="mt-4 p-4 bg-white rounded-lg">
                          <p className="text-sm text-gray-600">
                            <strong>Primary Contact:</strong> {safeContacts[0].name} ({safeContacts[0].role}) - {safeContacts[0].phone}
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-blue-50 p-8 rounded-xl border border-blue-200">
            <h4 className="text-xl font-semibold text-blue-800 mb-4">✅ Lead Management Best Practices</h4>
            <ul className="text-blue-700 space-y-2">
              {[
                'Always add complete contact information',
                'Update status regularly to reflect current relationship',
                'Assign appropriate agent category based on performance',
                'Use tags to categorize leads (e.g., "High Potential", "UK Specialist")',
                'Keep contact details updated and verified'
              ].map(practice => (
      ...
      content: (
        <div className="space-y-8">
          <div className="bg-gradient-to-r from-orange-50 to-red-50 p-8 rounded-xl border border-orange-200">
            <h3 className="text-2xl font-bold text-orange-800 mb-4">Follow-ups & Meeting Management</h3>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 border rounded-xl shadow-sm">
              <h4 className="text-lg font-semibold text-gray-800 mb-4">📞 Follow-up Types</h4>
              <div className="space-y-3">
            ...
            <div className="bg-white p-6 border rounded-xl shadow-sm">
              <h4 className="text-lg font-semibold text-gray-800 mb-4">📊 Follow-up Status</h4>
              <div className="space-y-3">
                {[
                  { status: 'Planned', color: 'yellow' },
                  { status: 'Done', color: 'green' }
                ].map(item => (
                  <div key={item.status} className="flex items-center p-3 bg-gray-50 rounded-lg">
                    <span className={`w-3 h-3 rounded-full mr-3 bg-${item.color}-500`}></span>
            ...
            <div className="bg-white p-6 border rounded-xl shadow-sm">
              <h4 className="text-lg font-semibold text-gray-800 mb-4">📅 Meeting Features</h4>
              <div className="space-y-2">
                {[
          ...
          {sampleFollowUps.length > 0 && (
            <div className="bg-white border-2 border-gray-200 rounded-xl p-8">
              <h4 className="text-xl font-semibold text-gray-800 mb-6">📞 Live Follow-up Examples</h4>
              <div className="space-y-4">
                {sampleFollowUps.map((followUp, idx) => (
                  <div key={followUp.id || idx} className="border-l-4 border-orange-500 pl-6 py-4 bg-gray-50 rounded-lg">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <span className="text-lg font-semibold">{followUp.type}</span>
          ...
          <div className="bg-green-50 p-8 rounded-xl border border-green-200">
            <h4 className="text-xl font-semibold text-green-800 mb-4">✅ Follow-up Best Practices</h4>
            <ul className="text-green-700 space-y-2">
              {[
                'Schedule follow-ups immediately after initial contact',
                'Use different types (Call/Meeting/Email) based on context',
                'Always add detailed notes about the conversation',
                'Mark follow-ups as "Done" when completed',
                'Use meeting check-in for in-person meetings',
                'Take photos during meetings for documentation'
              ].map(practice => (
                <li key={practice} className="flex items-start">
